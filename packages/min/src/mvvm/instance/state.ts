...
function initProps (vm: any, propsOptions: Object) {
  ...
  for (const key in propsOptions) {
    ...
    // during MVVM.extend(). We only need to proxy props defined at
    // instantiation here.
    if (!(key in vm)) {
      proxy(vm, `_props`, key)
    }
...
function getData (data: Function, vm: any): any {
  try {
  ...
  for (const key in methods) {
    if (process.env.NODE_ENV !== 'production') {
      ...
      if ((key in vm) && isReserved(key)) {
        warn(
          `Method "${key}" conflicts with an existing MVVM instance method. ` +
...
export function stateMixin (MVVM: any) {
  ...
  Object.defineProperty(MVVM.prototype, '$data', dataDef)
  Object.defineProperty(MVVM.prototype, '$props', propsDef)

  MVVM.prototype.$set = set
  MVVM.prototype.$delete = del
